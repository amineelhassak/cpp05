...
AForm::AForm() 
    : name("_default"), isSigned(false), gradeToSign(150), gradeToExecute(150)
{
    std::cout << "AForme : " << this->name << " constructeur par défaut appelé " << std::endl;
}

AForm::AForm(const std::string &name)
    : name(name), isSigned(false), gradeToSign(150), gradeToExecute(150)
{
    std::cout << "I) AForme : " << this->name << " constructeur avec paramètre appelé " << std::endl;
}

AForm::AForm(const int toSign, const int toExecute) 
    : name("_default"), isSigned(false), gradeToSign(toSign), gradeToExecute(toExecute)
{
...
AForm::AForm(const AForm &_Aform) 
    : name(_Aform.name), gradeToSign(_Aform.gradeToSign), gradeToExecute(_Aform.gradeToExecute)
{
    *this = _Aform;
